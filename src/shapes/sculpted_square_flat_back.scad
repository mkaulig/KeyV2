...
module sculpted_square_flat_back_shape(size, delta, progress) {
  width = size[0];
  height = size[1];
  ...
  extra_height_this_slice = (height_difference - (extra_side_size/2)) * progress;
  ...
  offset(r = extra_corner_radius_this_slice, $fa=360/$shape_facets) {
    offset(r = -extra_corner_radius_this_slice) {
      translate([0, -(extra_side_size*progress)/4]) side_rounded_square(square_size, r = $more_side_sculpting_factor * progress);
    }
  }
}
...
module side_rounded_square(size, r) {
    ...
    union() {
      if (sr > 0) {
        ...
        // TODO hier muss die translation rein
